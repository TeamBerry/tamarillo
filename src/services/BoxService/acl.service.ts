...
import { FeedbackMessage, BoxScope, Permission, ACLEvaluationResult } from "@teamberry/muscadine"
const Box = require("../../models/box.model")

class ACLService {
    public async isAuthorized(scope: BoxScope, action: Permission): Promise<ACLEvaluationResult> {
        ...
        if (subscriber.role === 'admin') {
            return ACLEvaluationResult.YES
        }
        ...
        if (box.acl[subscriber.role].includes(action)) {
            ...
            if (box.options.berries){
                ...
                return berriesPermissions.includes(action) ? ACLEvaluationResult.BERRIES : ACLEvaluationResult.NO
            }
        }
        return ACLEvaluationResult.NO
    }
