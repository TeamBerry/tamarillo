...
describe("Box API", () => {
    ...
    describe("Gets all boxes", () => {
        it("Returns all public boxes for an anonymous request", () => supertest(expressApp)
            .get('/')
            .expect(200)
            .then(response => {
        ...
        it("Returns all public boxes, private boxes of the user requesting and private boxes of other people the requesting user had already entered before", () => supertest(expressApp)
            .get('/')
        ...
        it("Returns all boxes if super admin", () => supertest(expressApp)
            .get('/')
    ...
    describe("Gets a single box", () => {
        it("Sends a 404 back if no box matches the id given", () => supertest(expressApp)
    ...
    describe("Updates a box", () => {
        it("Sends a 412 back if no request body is given", () => supertest(expressApp)
    ...
    describe("Patch settings quickly", () => {
        it("Sends a 412 if no setting is given", async () => supertest(expressApp)
            .patch('/9cb763b6e72611381ef053e9')
            .set('Authorization', `Bearer ${ashJWT.bearer}`)
            .expect(412, 'MISSING_PARAMETERS'))

        it("Sends a 412 if something other than a box setting is given", async () => supertest(expressApp)
            .patch('/9cb763b6e72611381ef053e9')
            .send({ name: 'Updated Box' })
            .set('Authorization', `Bearer ${ashJWT.bearer}`)
            .expect(412, 'UNKNOWN_PARAMETER'))

        it("Sends a 404 if the box does not exist", () => supertest(expressApp)
        ...
        it("Sends a 200 with the updated settings", () => supertest(expressApp)
            .patch('/9cb763b6e72611381ef053e9')
            .send({ random: false })
            .set('Authorization', `Bearer ${ashJWT.bearer}`)
            .expect(200)
    ...
    describe("Generates an invite to a box", () => {
        ...
        it("Rejects if the box is closed", () => supertest(expressApp)
            .post("/9cb763b6e72611381ef043e5/invite")
            .set('Authorization', `Bearer ${ashJWT.bearer}`)
            .expect(403, 'BOX_CLOSED'))

        it("Generates a default invite link", () => supertest(expressApp)
        ...
        it("Generates an invite link", () => supertest(expressApp)
            .post("/9cb763b6e72611381ef043e4/invite")
            .set('Authorization', `Bearer ${ashJWT.bearer}`)
            .send({
                expiration: 900
    ...
    describe("Closes a box", () => {
        ...
        it("Sends a 200 with the closed box", () => supertest(expressApp)
    ...
    describe("Opens a box", () => {
        ...
        it('Sends a 200 with the opened box', () => supertest(expressApp)
    ...
    describe("Features a box", () => {
        it("Refuses if the action does not come from an admin", () => supertest(expressApp)
        ...
        it("Refuses if the box is closed", () => supertest(expressApp)
        ...
        it("Refuses if the box is private", () => supertest(expressApp)
        ...
        it("Features a box to the set date", () => supertest(expressApp)
            .put('/9cb763b6e72611381ef053e9/feature')
            .set('Authorization', `Bearer ${adminJWT.bearer}`)
            .send({
                // Featured for an hour
        ...
        it("Overwrites the date of an already featured box", () => supertest(expressApp)
        ...
        it("Removes the featuring if null is given", () => supertest(expressApp)
            .put('/9cb763b6e72611381ef053f0/feature')
            .set('Authorization', `Bearer ${adminJWT.bearer}`)
            .send({
                // Featured for an hour
    ...
    describe("Converts the box of a playlist to an user playlist", () => {
        ...
        it("Sends a 401 if there's no authentication", () => supertest(expressApp)
            .post('/9cb763b6e72611381ef043e8/convert')
            .expect(401, 'UNAUTHORIZED'))

        it("Sends a 401 if there's a playlist specified as target but is not the user's", () => supertest(expressApp)
            .post('/9cb763b6e72611381ef043e8/convert')
            .set('Authorization', `Bearer ${shironaJWT.bearer}`)
            .send({_id: '7dec3a584ec1317ade113a58'})
            .expect(401, 'UNAUTHORIZED'))

        it("Sends a 412 if the source box has no videos in its playlist", () => supertest(expressApp)
            .post('/9cb763b6e72611381ef043e8/convert')
            .set('Authorization', `Bearer ${ashJWT.bearer}`)
            .send({_id: '7dec3a584ec1317ade113a58'})
            .expect(412, 'EMPTY_PLAYLIST'))

        it("Updates a playlist with unique videos if an existing playlist is specified as target", () => supertest(expressApp)
